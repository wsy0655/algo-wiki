...
本题实际就是拆点最短路，需要记录到达每个点的编号和此时的方向。但由于每个点的度数不同，因此无法直接开一个 $n\times m$ 的二维数组。

由于 $\sum d_i=m$，因此第二维需要的不会太多，因此考虑使用 `map` 来进行更新拆点最短路。


- 定义 `map<int, int> dis[N]` 记录到每个点 $u$ 且方向为 $p$ 的最短路。
    - `map` 的初始化可以做到随用随初始化，可以参考如下代码。

```cpp
void init(int u, int p)
{
    if (!dis[u].count(p)) // 当前状态不存在
        dis[u][p] = 1e18;
}
```


- 定义 `int ans[N]` 记录到每个点 $u$ 的最小花费。
    - 初始化所有 $ans[i]=+\infty$。


优先队列维护 `(dis[u][p], u, p)`。分别代表距离，点的编号，方向。



  
太好了，我就按“**只讲思路** + **最后附上你这份 AC 代码**”来写，给学生看会更顺畅。
